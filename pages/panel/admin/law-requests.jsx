...
export default function AdminLawRequests() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch('/api/admin/law-requests');
  ...
  return (
    <PanelLayout role="admin">
      <Card>
        <CardHeader>
          <CardTitle>Zapytania prawne</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="mb-4 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="SUBMITTED">Nowe</option>
              ...
              <TableBody>
                {filteredRows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>{row.wantsMeeting ? 'Tak' : 'Nie'}</TableCell>
                    <TableCell>
      ...
      <Dialog open={!!descriptionPreview} onOpenChange={(open) => !open && setDescriptionPreview('')}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Pełny opis zapytania</DialogTitle>
          </DialogHeader>
